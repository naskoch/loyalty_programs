...
An interesting point to observe above is that maximizing the revenue objective is equivalent to minimizing the influence zone, \ie, minimizing the fraction of customer visits until when merchant should offer reward point accelerations also maximizes the revenue objective of the merchant. {\arpit Better? We can even remove it if it doesn't offer any relevant exposition} {\nolan It still seems like some words are missing - particularly in ``until when merchant should offer reward..." Are you saying minimizing zone of influence via reward point accelerations also maximizes revenue or because the two optimization points are equivalent then merchants should offer reward points acceleration? I think it is a good point though, so worth keeping.}. 
...
Thus, for any choice of $p$ and $v$, the combination of the two above lemmas gives an interval of $b$ values for which the reward program is the most profitable choice for the merchant. Figure~\ref{fig:b_region} shows the bounds on $b$ for varying values of $p$, keeping $v = 0.05$ fixed. For some values of $p$ and $v$, the bounds given by the lemmas may fall outside the interval $[0,1]$, but we $b$ must fall within this range, so we restrict the bounds to this interval in figure~\ref{fig:b_region}. Notice that the upper bound on $b$ increases as a function of $p$ while the lower bound decreases with $p$, so the interval of ``strictly better'' $b$ values increases with $p$. We formalize this observation in a following lemma. Finally, observe that the bounds on $b$ are not very intuitive functions, so we may also write simple conditions on $p$.
...
We observe this result in figure~\ref{fig:b_region}. Figure~\ref{fig:b_restrictions} shows the upper and lower bounds on $b$ for all valid pairs of $p$ and $v$ with $\alpha \rightarrow e$. The top plot shows the lower bound on $b$ and the bottom depicts the upper bound. For a particular $(p,v)$ pair, if the color on the top plot is darker than the corresponding color on the bottom plot, then this pair has a valid $b$ interval in which the reward program is strictly better. This figure also exhibits the increasing range of $b$ values with increasing $p$; for large values of $p$ and moderate values of $v$, we observe no restrictions on $b$ for the reward program to be strictly better. 
...
The above theorem combines our previous lemmas, but is only valid for $\alpha \rightarrow e$. Now we generalize the above result for all values of $\alpha$. The conditions are more complex but the results and intuitions are similar. 
...
The above proposition and discussion proves our lemma: for fixed $\alpha$ and any $(p,v)$ pair, there is some upperbound on $b$ s.t. $RoR_A > \frac{b}{2}$. Now again we look at conditions for $RoR_A > RoR_B$ to get a lower bound on $b$.  
...
We may combine the above lemmas to get an interval of $b$ values on which the reward program is strictly better. However, we do not have exact expressions for these bounds for each $(p,v)$ pair. 
