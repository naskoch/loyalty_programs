...
That is, the region where $A$ earns higher revenue rate as compared to $B$ and the region where it is profitable for $A$ to run a reward program as opposed to running traditional pricing. 
...
\begin{figure}[h!]
\begin{centering}
\includegraphics[scale = 0.75]{./figures/b_lowerbound.png}
\caption{Lower bound on $b$ for $RoR_A > RoR_B$ for different values of $p$ and $v$ with $\alpha \rightarrow e$.}
...
{\nolan May be worth adding both figures together in python as well, but we can worry about that later if we want.}
