...
{\nolan Probably want some exposition between the lemma and the theorem.}
...
{\nolan Don't think the above is necessary anymore? But will keep for now.}
...
Just as we can look at values of $b$ and $p$ such that $RoR_A > RoR_B$, we can characterize the $(b,p)$ pairs for which $RoR_A > \frac{b}{2}$. These pairs correspond to customer distributions for which a reward program is more profitable than not. Figure~\ref{fig:offer_reward_or_not} shows the pairs for different values of $\alpha$, keeping $v = 0.05$ and $\beta = 0.95$ fixed. Observe that under these conditions, the firm should choose to run a reward program most of the time. Also notice that reward programs begin to lose profitability as $b$ increases; larger $b$ values mean that people make more exogenous visits, so a reward program is no longer needed to entice visits.
...
\begin{subfigure}[t]{0.5\textwidth}
\centering
\includegraphics[height=2in]{./figures/bp_pair_rorA_al0p5_dense.png}
\caption{$\alpha = 0.5$}
\end{subfigure}%
~ 
\begin{subfigure}[t]{0.5\textwidth}
\centering
\includegraphics[height=2in]{./figures/bp_pair_rorA_al1p0_dense.png}
\caption{$\alpha = 1$}
\end{subfigure}
\centering
\begin{subfigure}[t]{0.5\textwidth}
\centering
\includegraphics[height=2in]{./figures/bp_pair_rorA_al2p0_dense.png}
\caption{$\alpha = 2$}
\end{subfigure}%
~ 
\begin{subfigure}[t]{0.5\textwidth}
\centering
\includegraphics[height=2in]{./figures/bp_pair_rorA_al2p5_dense.png}
\caption{$\alpha = 2.5$}
\end{subfigure}

\caption{Achievable region of $(b,p)$ pairs where rate of revenue is higher with a reward program than it is without. In these plots $v = 0.05$ and $\beta = 0.95$.}
\label{fig:offer_reward_or_not}
\end{figure*}

For any fixed $\alpha$, the exact conditions on $p$, $b$ and $v$ for $RoR_A > \frac{b}{2}$ are rather complex. We will focus on one particular simple case: $\alpha \rightarrow e$. 
...
\begin{subfigure}[t]{0.5\textwidth}
\centering
\includegraphics[height=2in]{./figures/bp_pair_both_al0p5_dense.png}
\caption{$\alpha = 0.5$}
\end{subfigure}%
~ 
\begin{subfigure}[t]{0.5\textwidth}
\centering
\includegraphics[height=2in]{./figures/bp_pair_both_al1p0_dense.png}
\caption{$\alpha = 1$}
\end{subfigure}
\centering
\begin{subfigure}[t]{0.5\textwidth}
\centering
\includegraphics[height=2in]{./figures/bp_pair_both_al2p0_dense.png}
\caption{$\alpha = 2$}
\end{subfigure}%
~ 
\begin{subfigure}[t]{0.5\textwidth}
\centering
\includegraphics[height=2in]{./figures/bp_pair_both_al2p5_dense.png}
\caption{$\alpha = 2.5$}
\end{subfigure}

\caption{{\nolan Just adding these in for reference. I think if we do want to do both together, we should not have them be so dense, or we need to do some other method of plotting. Also, not sure these are the best representatives, maybe we keep the other plots and then plot both together for different $v$'s and $\alpha$'s?}}
\label{fig:offer_reward_or_not}
\end{figure*}
