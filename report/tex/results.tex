...
So far we have characterized the consumer behavior within the duopoly without concern about the particular reward parameters. In this section, we derive optimal parameters for these parameters with respect to the revenue of the reward merchant. Interestingly, we see that maximizing revenue corresponds to minimizing the influence zone.
...
{\arpit We show in Figure~\ref{} about numerical results on optimal alpha}{\nolan I will look into this and if we do find something, we can expand above paragraph a little.}

\subsubsection{Revenue Comparisons}
{\nolan maybe need to rethink titles of sections, but using these as placeholders.}

Now we characterize conditions for when it is strictly better for $A$ to offer a reward program for a specific distribution of the loyalty bias paramter - when $\lambda$ for every customer is drawn uniformly at random between $(0,b]$ where $b$ is less than $1$. We will assume this distribution for the remainder of the section.
...
First we evaluate the expected rates of revenue for both $A$ and $B$ under these conditions of proportional budgeting and the optimal relation between $k$ and $\alpha$ as shown in the previous theorem with $\lambda$ drawn from a uniform distribution.
