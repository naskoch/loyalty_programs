...
This behavior is cyclic, and repeats after every reward redemption. This behavior occurs because $V$ is an increasing funcion of $i$ under mild assumptions on our model parameters.
...
\proof
First we show that $V(i)$ is an increasing function in $i$ by induction. We begin by  showing that if the condition above is satisfied, $V(k-1) < V(k) = R$. Suppose not, so $V(i) \geq R$. Then we have:
\begin{align*}
...
We refer to the ratio of the number of visits required for a strategic customer to adopt a reward program and the total distance to the reward as the ``influence zone''.
Intuitively this is the fraction of visits that the merchant wants to influence the customer by offering exogenous means of earning additional points like bonus miles in airlines or accelerated earnings.
...
Influence zone is minimized at $k = \frac{e}{\alpha(1-\beta)}$ under constant proportional promotion budgeting as long as $\beta$ is close to 1. 
\end{remark}
\proof
