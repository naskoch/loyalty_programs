...
Now we characterize conditions for when it is strictly better for $A$ to offer a reward program for a specific distribution of the loyalty bias paramter - when $\lambda$ for every customer is drawn uniformly at random between $(0,b]$ where $b$ is less than $1$.
...
First we evaluate the expected rate of revenues for both $A$ and $B$ under these conditions of proportional budgeting and optimal relation between $k$ and $\alpha$ as shown in the above theorem.
...
{\nolan I may rearrange some of the proof text to outside the proof - just a reminder to myself.}
