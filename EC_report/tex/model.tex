...
\item
With probability $1-\lambda$, the customer has a choice between purchasing from $B$ for an immediate utility $v$ and remain in state $i$, or purchasing from $A$ for no immediate utility but move to state $i+1$.
\end{enumerate}
...
\eeq
\beq
RoR_B(c) = \underset{\tau, t, \lambda}E\left[\frac{(1-v)(\tau(t,\lambda) - i_0(t))}{\tau(t,\lambda) + k - i_0(t)}\right]\notag
\eeq
...
\beq
RoR_A = \underset{t, \lambda}E\left[\frac{k-R}{i_0(t)/\lambda + k - i_0(t)}\right]
\eeq

\beq
RoR_B = \underset{t, \lambda}E\left[\frac{(i_0(t)\lambda - i_0(t))(1-v)}{i_0(t)/\lambda + k - i_0(t)}\right]
\eeq
